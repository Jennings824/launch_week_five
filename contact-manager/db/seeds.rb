...
contact_attributes.each do |attributes|
  persisted_contact = Contact.
  .find_by(first_name: attributes[:first_name], last_name:attributes[:last_name])
  if persisted_contact.nil?
  Contact.create(attributes)
else
  persisted_contact.phone_number = attribute[:phone_number]
